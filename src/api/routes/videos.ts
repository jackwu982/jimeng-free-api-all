  ...
  post: {
    "/generations": async (request: Request) => {
      ...
      if (foundUnsupported.length > 0) {
        throw new Error(
          `不支持的参数: ${foundUnsupported.join(", ")}。请使用 ratio 和 resolution 参数控制视频尺寸。`,
        );
      }
      ...
      const finalDuration =
        isMultiPart && typeof duration === "string"
          ? parseInt(duration)
          : duration;
    ...
    "/query": async (request: Request) => {
      request
        .validate("headers.authorization", _.isString)
        .validate("query.historyId", _.isString);
