...
import {
  ...
  receiveCredit,
  request,
  DEFAULT_ASSISTANT_ID as CORE_ASSISTANT_ID,
  WEB_ID,
  acquireToken,
} from "./core.ts";
...
export function isSeedanceModel(model: string): boolean {
  return (
    model.startsWith("seedance-") || model.startsWith("jimeng-video-seedance-")
  );
}
...
function resolveVideoResolution(
  ...
  if (!resolutionGroup) {
    ...
    throw new Error(
      `不支持的视频分辨率 "${resolution}"。支持的分辨率: ${supportedResolutions}`,
    );
  }
...
async function uploadImageForVideo(
  ...
  try {
    ...
    if (!applyResponse.ok) {
      ...
      throw new Error(
        `申请上传权限失败: ${applyResponse.status} - ${errorText}`,
      );
    }
    ...
    if (
      ...
      commitResult.Result.Results.length === 0
    ) {
...
// 从Buffer上传视频图片
async function uploadImageBufferForVideo(
  buffer: Buffer,
  refreshToken: string,
): Promise<string> {
  try {
...
async function uploadMediaForVideo(
  ...
  const { access_key_id, secret_access_key, session_token, space_name } =
    tokenResult;
  if (!access_key_id || !secret_access_key || !session_token) {
  ...
  if (!applyResponse.ok) {
    const errorText = await applyResponse.text();
    throw new Error(
      `申请${label}上传权限失败: ${applyResponse.status} - ${errorText}`,
    );
  }
  ...
  if (applyResult?.ResponseMetadata?.Error) {
    throw new Error(
      `申请${label}上传权限失败: ${JSON.stringify(applyResult.ResponseMetadata.Error)}`,
    );
  }
  ...
  if (!storeInfo) {
    throw new Error(
      `获取${label}上传存储信息失败: ${JSON.stringify(uploadNode)}`,
    );
  }
  ...
  if (!uploadResponse.ok) {
    const errorText = await uploadResponse.text();
    throw new Error(
      `${label}文件上传失败: ${uploadResponse.status} - ${errorText}`,
    );
  }
  ...
  if (uploadData?.code !== 2000) {
    throw new Error(
      `${label}文件上传失败: code=${uploadData?.code}, message=${uploadData?.message}`,
    );
  }
  ...
  if (!commitResponse.ok) {
    const errorText = await commitResponse.text();
    throw new Error(
      `提交${label}上传失败: ${commitResponse.status} - ${errorText}`,
    );
  }
  ...
  if (commitResult?.ResponseMetadata?.Error) {
    throw new Error(
      `提交${label}上传失败: ${JSON.stringify(commitResult.ResponseMetadata.Error)}`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `提交${label}上传响应缺少结果: ${JSON.stringify(commitResult)}`,
    );
  }
 ...
 */
async function fetchHighQualityVideoUrl(
  itemId: string,
  refreshToken: string,
): Promise<string | null> {
  try {
    ...
    const hqUrlMatch = responseStr.match(
    ...
    const jimengUrlMatch = responseStr.match(
    ...
    // 策略4: 匹配任何视频URL（兜底）
    const anyVideoUrlMatch = responseStr.match(
...
export async function generateVideo(
  ...
  logger.info(
    `使用模型: ${_model} 映射模型: ${model} ${width}x${height} (${ratio}@${resolution}) 时长: ${duration}秒`,
  );
  ...
  if (files && files.length > 0) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        logger.info(
          `开始上传第 ${i + 1} 个文件: ${file.originalFilename || file.filepath}`,
        );
  ...
  logger.info(
    `开始轮询视频生成结果，历史ID: ${historyId}，最大重试次数: ${maxRetries}`,
  );
  logger.info(
    `即梦官网API地址: https://jimeng.jianying.com/mweb/v1/get_history_by_ids`,
  );
  logger.info(
    `视频生成请求已发送，请同时在即梦官网查看: https://jimeng.jianying.com/ai-tool/video/generate`,
  );
  ...
  while (status === 20 && retryCount < maxRetries) {
    try {
      ...
      if (useAlternativeApi) {
        // 备用API请求方式
        logger.info(
          `尝试备用API请求方式，URL: ${requestUrl}, 历史ID: ${historyId}, 重试次数: ${retryCount + 1}/${maxRetries}`,
        );
        ...
        logger.info(
          `备用API响应摘要: ${JSON.stringify(result).substring(0, 500)}...`,
        );
      } else {
        // 标准API请求方式
        logger.info(
          `发送请求获取视频生成结果，URL: ${requestUrl}, 历史ID: ${historyId}, 重试次数: ${retryCount + 1}/${maxRetries}`,
        );
      ...
      if (
        ...
        result.history_records.length > 0
      ) {
        // 处理备用API返回的数据格式
        ...
        // 所有API都没有返回有效数据
        logger.warn(
          `历史记录不存在，重试中 (${retryCount + 1}/${maxRetries})... 历史ID: ${historyId}`,
        );
        logger.info(
          `请同时在即梦官网检查视频是否已生成: https://jimeng.jianying.com/ai-tool/video/generate`,
        );
      ...
      logger.info(
        `视频生成状态: ${status}, 失败代码: ${failCode || "无"}, 项目列表长度: ${item_list.length}`,
      );
      ...
      if (status === 20) {
        ...
        logger.info(
          `视频生成中，状态码: ${status}，等待 ${waitTime}ms 后继续查询`,
        );
        await new Promise((resolve) => setTimeout(resolve, waitTime));
      }
    } catch (error) {
  ...
  if (retryCount >= maxRetries && status === 20) {
    logger.error(
      `视频生成超时，已尝试 ${retryCount} 次，总耗时约 ${Math.floor((retryCount * 2000) / 1000 / 60)} 分钟`,
    );
    const error = new APIException(
      EX.API_IMAGE_GENERATION_FAILED,
      "获取视频生成结果超时，请稍后在即梦官网查看您的视频",
    );
    // 添加历史ID到错误对象，以便在chat.ts中显示
    error.historyId = historyId;
    throw error;
  ...
  if (itemId) {
    try {
      const hqVideoUrl = await fetchHighQualityVideoUrl(
        String(itemId),
        refreshToken,
      );
      if (hqVideoUrl) {
      ...
      logger.warn(
        `获取高质量视频URL失败，将使用预览URL作为回退: ${error.message}`,
      );
    }
  } else {
    logger.warn(
      `未能从item_list中提取item_id，将使用预览URL。item_list[0]键: ${item_list?.[0] ? Object.keys(item_list[0]).join(", ") : "无"}`,
    );
  }
...
export async function generateSeedanceVideo(
  ...
  logger.info(
    `Seedance 2.0 生成: 模型=${_model} 映射=${model} ${width}x${height} (${ratio}@${resolution}) 时长=${actualDuration}秒`,
  );
  ...
  if (files && files.length > 0) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        logger.info(
          `Seedance: 开始上传第 ${i + 1} 个文件 (${materialType}): ${file.originalFilename || file.filepath}`,
        );
        const buffer = fs.readFileSync(file.filepath);

        if (materialType === "image") {
          const imageUri = await uploadImageBufferForVideo(
            buffer,
            refreshToken,
          );
          if (imageUri) {
          ...
          // 视频或音频 → VOD 上传
          const vodResult = await uploadMediaForVideo(
            buffer,
            materialType,
            refreshToken,
            file.originalFilename,
          ...
          logger.info(
            `Seedance: 第 ${i + 1} 个${materialType === "video" ? "视频" : "音频"}上传成功: ${vodResult.vid}`,
          );
        }
      } catch (error) {
    ...
    for (let i = 0; i < filePaths.length; i++) {
      ...
      try {
        logger.info(
          `Seedance: 开始上传第 ${i + 1} 个文件 (${materialType}): ${filePath}`,
        );
  ...
  if (uploadedMaterials.length === 0) {
    throw new APIException(
      EX.API_REQUEST_FAILED,
      "Seedance 2.0 需要至少一个文件（图片/视频/音频）",
    );
  }
    ...
    sceneOptions: JSON.stringify([
      {
        ...
        materialTypes: [
          ...new Set(uploadedMaterials.map((m) => MATERIAL_TYPE_CODE[m.type])),
        ],
      },
    ]),
  });
...
export async function getVideoUrlByHistoryId(
  historyId: string,
  refreshToken: string,
) {
  // 轮询获取结果（与普通视频相同的逻辑）
  ...
  if (seedanceItemId) {
    try {
      ...
      logger.warn(
        `Seedance: 获取高质量视频URL失败，将使用预览URL作为回退: ${error.message}`,
      );
    }
  } else {
    logger.warn(
      `Seedance: 未能从item_list中提取item_id，将使用预览URL。item_list[0]键: ${item_list?.[0] ? Object.keys(item_list[0]).join(", ") : "无"}`,
    );
  }
 ...
 */
function buildMetaListFromPrompt(
  prompt: string,
  materials: Array<{ type: SeedanceMaterialType }>,
): Array<{
  meta_type: string;
  text?: string;
